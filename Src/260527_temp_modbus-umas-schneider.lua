...
Memory_Address       = ProtoField.uint32("UMAS.Memory_Address"     , "Memory_Address"     , base.HEX) --This
Block_Size           = ProtoField.uint16("UMAS.Block_Size"         , "Block_Size"         , base.DEC) --This

Umas_Data               = ProtoField.string("UMAS.Umas_Data"            , "Umas_Data"            , base.ASCII )

umas_protocol.fields    = { Function_Code, Pairing_Key, Functions_Code , Umas_Functions_Code, Memory_Address, Block_Size ,Umas_Data } --This
-------------------------------------------------------
...
--This
        if (umas_code == 40 or umas_code == 41) and length >= 16 then
            umasSubtree:add_le(Memory_Address, buffer(10,4))
            umasSubtree:add_le(Block_Size,     buffer(14,2))
            if length > 16 then
                umasSubtree:add(Umas_Data, buffer(16), buffer(16):le_ustring())
            end
        else
            umasSubtree:add(Umas_Data, buffer(10), buffer(10):le_ustring())
        end
